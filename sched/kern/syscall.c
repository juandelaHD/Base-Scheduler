...
sys_exofork(void)
{
	...
	newenv->env_pri =
	        curenv->env_pri;  // el hijo hereda la prioridad del padre
...
void
sys_env_mod_priority(envid_t envid)
{
	struct Env *env;
	envid2env(envid, &env, 0);
	if (env->env_pri != 0) {
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_env_get_priority:
		sys_env_get_priority(a1);
	case SYS_env_mod_priority:
		sys_env_mod_priority(a1);
	default:
		return -E_INVAL;
	}
}
